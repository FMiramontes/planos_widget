...
const UI = {
  ...
  async getSVG() {
        ...
        block.forEach((manzana) => {
          if (manzana?.path)
            containerManzanas.insertAdjacentHTML(
              'beforeend',
              `<g class="cls-1" id="${manzana.Numero}" data-conatiner="${
                manzana.Numero
    ...
    desarrollo.details.forEach((detail) => {
      detail.forEach((d) => {
        mapsDetails.insertAdjacentHTML('beforeend', this.parseOuterHTML(d))
      })
    })
  },
  ...
  getKeysForm() {
    ...
    Blocks.forEach((block) => {
      const spans = Array.from(block.children)
      spans.forEach((span) => {
        if (block?.dataset?.contacto) contacto.push(span.children[1].name)

        if (block?.dataset?.presupuesto) Presupuesto.push(span.children[1].name)
      })
    })
  ...
  async validate(CRMData, newData) {
    ...
    const Apartado = document.querySelector('input[name="Cantidad_RA"]').value
    const Enganche = document.querySelector('input[name="Enganche_P"]').value
    ...
    if (temp_contact_id == false) {
      ...
      if (conatctRequest.ok) {
        ...
        accountId = createAccountRequest?.data?.details?.id
        // Create Contact
        const createContactRequest = await crm.CreateContact(newData, accountId)
    ...
    if (sku === undefined) {
      ...
      const data = await crm.detailsFraccionamiento(map.dataset?.commerceId)
      ...
      if (createCotizacion.ok) {
        ...
        if (CotizacionRequest.ok) {
          ...
          for (let i = 0; i < arrInvoices.length; i++) {
            ...
            console.log(`UI createInvoice: index ${i}', ${invoiceRequest}`)
          }
  ...
  selectContact(selectedOption) {
    ...
    const contactContainerResults = document.querySelector('#contact-results')
  ...
  async searchCampaign() {
    ...
    // console.log('searchValue', searchValue)
    const campaignResultContainer = document.querySelector('#campaign-results')
  ...
  selectCampaign(selectedOption) {
    ...
    campaignInput.dataset.diferido = selectedOption.children[0].dataset.diferido
  ...
  async fillCampaignDetails() {
    ...
    const fieldApartado = document.querySelector(`input[name="Cantidad_RA"]`)
    ...
    if (campaignId !== null) {
      ...
      if (getCampaignRequest.ok) {
        ...
        const COSTO_M2 = document.querySelector(`input[name="Costo_M2"]`).value
        ...
        if (tipoDeDescuento != null) {
          if (tipoDeDescuento == 'Monto') {
            ...
            let porcentajeDescuento = campaignData.Porcentaje_Descuento / 100
            let descuentoPorcentaje =
              DIMENSIONES * COSTO_M2 * porcentajeDescuento
            precioFinalProducto = DIMENSIONES * COSTO_M2 - descuentoPorcentaje
          }
        } else {
        ...
        if (formaDePago != 'Contado') {
          ...
          if (politicaCampana == 'Primer Mensualidad') {
            ...
            if (tipoEnganche == 'Monto') {
              ...
              const porcEnganche = campaignData.Porcentaje_de_Enganche / 100
